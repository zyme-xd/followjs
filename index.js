  ...
  for (i = 0; info.length > i; i++) {
    if (new_ids.indexOf(info[i].id_str) == -1) {
      ...
      if (funkyinfo.errors[0].code == 162) {
        ...
        reason = 'Blocked'
      }} catch (error) {console.log('ya they aint block u or sum')}
      try { let suscheck = await postalt('friendships/create', {user_id: info[i].id_str})
    ...
    }} catch (error) {console.log('they aint suspended')}
       function dm(){
